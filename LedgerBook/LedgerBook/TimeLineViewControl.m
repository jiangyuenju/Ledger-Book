...
@property(nonatomic, strong) NSArray *ledgerTypes;
@end
...
- (id)initWithTimeArray:(NSArray *)time andTimeDescriptionArray:(NSArray *)timeDescriptions andTypeArray:(NSArray *) types andCurrentStatus:(int)status andFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        viewheight = 75.0;
         ...
         */
        self.ledgerTypes =[[NSArray alloc]initWithArray:types];
...
- (void)addTimeDescriptionLabels:(NSArray *)timeDescriptions andTime:(NSArray *)time currentStatus:(int)currentStatus {
    ...
    for (NSString *timeDescription in timeDescriptions) {
        ...
        [label makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(_progressDescriptionViewContainer).with.offset(15);
...
- (NSString*) getImageName:(int) index{
    NSString *type=[self.ledgerTypes objectAtIndex:index];
    NSString *result=nil;
    if([type isEqualToString:@"开始"]){
        result=@"start.png";
    }
    else if([type isEqualToString:@"餐饮"]){
        result = @"food.png";
    }
    ...
    else if([type isEqualToString:@"手机通讯"]){
        result=@"phone.png";
    }
    else if([type isEqualToString:@"娱乐"]){
        result=@"entertainment.png";
    }
    else if([type isEqualToString:@"书籍"]){
        result=@"book2.png";
    }
    else if([type isEqualToString:@"借出"]){
        result=@"borrow-out.png";
    }
    else if([type isEqualToString:@"人情礼物"]){
        result=@"gift";
    }
    ...
    else if([type isEqualToString:@"美容运动"]){
        result=@"beauty.png";
    }
    else if([type isEqualToString:@"其他"]){
        result=@"more.png";
    }
    else if([type isEqualToString:@"工资"]){
        result=@"work.png";
    }
    else if([type isEqualToString:@"奖金"]){
        result=@"bonus.png";
    }
    else if([type isEqualToString:@"外快兼职"]){
        result=@"part-time.png";
    }
    else if([type isEqualToString:@"借入"]){
        result=@"borrow-in.png";
    }
    else if([type isEqualToString:@"投资收入"]){
        result=@"invest.png";
    }
    else if([type isEqualToString:@"红包"]){
        result=@"red-pocket.png";
    }
    else if([type isEqualToString:@"零花钱"]){
        result=@"pocket-money.png";
    }
    else if([type isEqualToString:@"生活费"]){
        result=@"living-money.png";
    }
    return result;
}
...
//        [self.progressViewContainer.layer addSublayer:grayStaticCircleLayer];
        ...
        mylayer.position=CGPointMake(self.progressViewContainer.center.x+30, yCenter);
        NSString * name=[self getImageName:i];
        
        mylayer.contents=(id)[UIImage imageNamed:name].CGImage;
        [self.progressViewContainer.layer addSublayer:mylayer];
        //configure line
        if (i > 0) {
            fromPoint = lastpoint;
